    ...
    for block in &lowered.source_file.blocks {
        for statement in &block.statements {
            let (id, text, kind, header_line, target_line) = match statement {
                Statement::Line(line)
                    if line.statements.is_empty() && line.plural_source_text.is_none() =>
                {
                    (
                        &line.source_id,
                        &line.source_text,
                ...
                Statement::Choice(choice)
                    if choice.statements.is_empty()
                        && choice.availability_requirement.is_none()
                        && choice.availability_reason_override.is_none() =>
                {
                    (
                        &choice.source_id,
                        &choice.source_text,
                        PassageKind::Choice {
                            destination: choice.target.as_ref().map(|target| {
                                match &target.target {
                                    DivertTarget::End => "END".to_owned(),
                                    DivertTarget::Block(reference) => {
                                        reference.file.as_ref().map_or_else(
                ...
                Statement::Divert(divert) if matches!(divert.target, DivertTarget::End) => continue,
                _ => {
                    return Err(EditError::SourceRequired(
                        "conditions, effects, nested statements, or plurals are not projected yet",
                    ));
                }
            };
