use recite_bakeoff_authoring::{Document, EditError, FIXTURE, PassageKind, Preview};
...
fn real_document_names_separate_generated_choice_ids() -> Result<(), Box<dyn std::error::Error>> {
    let mut first = recite_bakeoff_authoring::Workbench::open("first.recite", FIXTURE)?;
    let mut second = recite_bakeoff_authoring::Workbench::open("second.recite", FIXTURE)?;
    first.add_choice()?;
    second.add_choice()?;
    ...
    let invalid =
        recite_bakeoff_authoring::Workbench::open("invalid.recite", ":: bad\n> broken@bad\n")?;
    assert_eq!(invalid.view(), &recite_bakeoff_authoring::View::Source);
    Ok(())
}
