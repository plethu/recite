...
ensuring that the encoder emits a representable current or legacy row shape.
Current rows retain the 9-field and 11-field bytes. Legacy decoded rows use the
already-supported 5-field line and 9-field choice shapes when their authored
and decoded text match and they have no bindings or plural fields.

The public error contract is the typed `CompiledAssetDecodeError` variant (and
the corresponding typed encode error), not the diagnostic detail string inside
`MalformedAsset` or `InvalidDialogue`. Detail wording and the precedence of
simultaneous malformed fields may change as the shared authority evolves; they
are deliberately not wire or compatibility contracts.
...
| `ContentFingerprint`: `algorithm`, `digest` | `V0_FINGERPRINT_FIELDS` (2) | `MsgFingerprint` | structural/newtype conversion; fingerprint constructor and digest-length checks | `json_fingerprint`; core fingerprint tests | §12.2 fingerprint rules |
...
| `CompiledLine`: current `id`, decoded `source_text`, `speaker`, `metadata`, `source_map`, authored `source_text`, `interpolation_bindings`, optional decoded `plural_source_text`, optional authored `plural_source_text`; legacy `id`, `source_text`, `speaker`, `metadata`, `source_map` | `V0_LINE_FIELDS` (9) current; legacy arity 5 | `MsgLine` current or legacy shape | structural `MsgLine` conversion; shared `validate_lines` interpolation/plural/index checks | `json_dialogue.lines`; core-language fixture and plural/legacy round-trip tests | §12.2 `CompiledLine` |
| `CompiledChoice`: current `id`, decoded `source_text`, `metadata`, `requirement`, `requirement_source_text`, `availability_reason_override`, `target`, `echo`, `source_map`, authored `source_text`, `interpolation_bindings`; legacy omits authored text and bindings | `V0_CHOICE_FIELDS` (11) current; legacy arity 9 | `MsgChoice` current or legacy shape | structural `MsgChoice` conversion; shared `validate_choices` interpolation/requirements/ranges | `json_dialogue.choices`; availability-reason and legacy fixture | §12.2 `CompiledChoice` |
...
| `CompiledSpeaker`: `id` | `V0_SPEAKER_FIELDS` (1) | custom `MsgSpeaker` serializer | structural tuple/newtype conversion; `SpeakerId` construction | `json_dialogue.speakers`; core-language fixture | §12.2 `CompiledSpeaker` |
...
| Lookup entry: `id`, `index` (for each of `block_lookup`, `line_lookup`, `choice_lookup`) | `V0_LOOKUP_ENTRY_FIELDS` (2) | `MsgLookupEntry` | structural conversion; sorted lookup construction; shared table consistency checks | `json_dialogue.*_lookup`; core lookup tests | §12.2 lookup entries |
