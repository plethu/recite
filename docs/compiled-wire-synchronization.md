...
| `CompiledAvailabilityReasonArgValue`: `tag`, `payload` (string-tagged argument value) | `V0_TAGGED_VALUE_FIELDS` (2) | `MsgAvailabilityReasonArgValue` in `core/src/compiled/messagepack/encode/tables.rs` (compiler delegates) | `MsgAvailabilityReasonArgValueWrapper` custom visitor in `core/src/compiled/messagepack/wire.rs`; tag and finite-float checks | `json_availability_reason_arg_binding`; literal-reason fixture, raw-tag guard, and literal-reason golden bytes | §12.2 availability-reason argument value |
