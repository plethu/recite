...
| `scripts/check-zed.sh` | 326 | tooling | zed/check | review | #192: Zed manifest, grammar pin, task argv, launcher API, parity evidence, and hostile code-action regression remain one checked boundary; split before adding another capability/acquisition family |
| `scripts/check-zed-host.sh` | 699 | tooling | zed/host | exception | #192: pinned Zed package setup, extension inspection, LSP/input probes, and process cleanup share one Linux host acceptance runner; split package setup from host phases before adding another capability/acquisition family |
...
| `tests/editor-hosts/vscode/host-probe.cjs` | 481 | test/support | vscode/host-tests | review | #51/#202: installed VS Code/VSCodium extension API and keyboard probe scenarios remain one host contract suite; split before another host interaction family |
| `tests/editor-parity/check.sh` | 498 | test/support | editor-parity/tests | review | #202: installed-host and incremental host-evidence fixture scenarios remain one executable editor-parity contract suite; split before another host evidence family |
| `tests/editor-parity/mutate_fixture.py` | 418 | test/support | editor-parity/tests | review | #202: editor-parity contract mutations for host records and evidence boundaries remain one fixture mutation suite; split before another evidence family |
