...
| `crates/recite-lsp/src/tests/project_indexes/discovery_transitions/fallback.rs` | 318 | test/support | lsp/tests | cohesive | Fallback ownership and source enumeration transitions remain together; failure-isolation and nested-manifest transitions are split into a sibling test family |
