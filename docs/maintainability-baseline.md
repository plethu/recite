...
| `editors/vscode/src/lsp-features.js` | 266 | production | vscode/lsp-features | review | #51: LSP feature, range/workspace-edit conversion, and version precondition projection remain one checked editor boundary |
...
| `scripts/check-vscode-host.sh` | 774 | tooling | vscode/host | exception | #51/#202: pinned VS Code/VSCodium acquisition, VSIX setup, API/keyboard probes, and process cleanup share one Linux host acceptance runner; split acquisition and provisioning from host phases before further growth |
...
| `scripts/check-zed-host.sh` | 676 | tooling | zed/host | exception | #192: pinned Zed package setup, extension inspection, LSP/input probes, and process cleanup share one Linux host acceptance runner; split package setup from host phases before further growth |
| `scripts/editor_parity/contract.py` | 261 | tooling | editor-parity/contract | review | #202: editor-parity contract topology and documentation validation remain one checked contract boundary |
| `scripts/editor_parity/evidence.py` | 296 | tooling | editor-parity/evidence | review | #202: editor-parity evidence commands, host records, and capability validation remain one checked contract boundary |
...
| `tests/editor-hosts/vscode/host-probe.cjs` | 461 | test/support | vscode/host-tests | review | #51/#202: installed VS Code/VSCodium extension API and keyboard probe scenarios remain one host contract suite; split before another host interaction family |
| `tests/editor-parity/check.sh` | 495 | test/support | editor-parity/tests | review | #202: installed-host and incremental host-evidence fixture scenarios remain one executable editor-parity contract suite; split before another host evidence family |
| `tests/editor-parity/mutate_fixture.py` | 413 | test/support | editor-parity/tests | review | #202: editor-parity contract mutations for host records and evidence boundaries remain one fixture mutation suite; split before another evidence family |
