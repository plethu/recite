...
async function runFinite(event, exitCode, terminal) {
  const child = new FakeChild();
  const promise = runFiniteWithChild(child);
  queueMicrotask(() => {
    child.stdout.emit("data", Buffer.from(JSON.stringify({
      ...
      version: 1, sequence: 1, event, command: "validate", invocation_id: "id", ...terminal
    }) + "\n"));
...
function runFiniteWithChild(child) {
  return runFiniteCommand({
    command: "recite", commandName: "validate", args: [], cwd: "/project", invocationId: "id",
    spawnProcess: () => child
  });
}
