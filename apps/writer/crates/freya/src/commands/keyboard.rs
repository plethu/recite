...
pub(crate) fn keyboard(
    ...
    if super::vim::modifier_keyboard(writer, &event) {
        return;
    }
    if event.modifiers == Modifiers::ALT && matches!(event.code, Code::ArrowLeft | Code::ArrowRight)
    {
        ...
        let regions = regions(writer, source, editor_id);
        if regions.is_empty() {
            return;
        }
...
pub(super) fn regions(
    ...
    if *writer.layout.navigation.peek()
        && !(*writer.layout.focus.peek()
