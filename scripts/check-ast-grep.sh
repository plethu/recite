...
Use --full to scan all tracked crates/tests Rust sources.
EOF
}
...
if ! command -v ast-grep >/dev/null 2>&1; then
  ...
  echo "Install the repository toolchain with: mise install" >&2
  exit 2
fi
...
if (( full_scan )); then
  ...
  while IFS= read -r -d '' path; do
    paths+=("$path")
  done < <(git -C "$repo_root" diff --name-only -z --diff-filter=ACMR "${base_sha}...${head_sha}")
fi

declare -a scan_paths=()
for path in "${paths[@]}"; do
  [[ "$path" == *.rs ]] || continue
  if [[ "$path" != crates/*/src/* ]]; then
    continue
  fi
