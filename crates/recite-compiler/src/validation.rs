...
use recite_core::{
    ...
    DivertTarget, Effect, IfBranch, Line, MatchArm, MatchBranch, Metadata, SourceFile,
    SourcePosition, SourceSpan, SourceText, Statement,
};
...
fn source_files_in_project_order(source_files: &[SourceFile]) -> Vec<&SourceFile> {
...
fn collect_blocks<'a>(source_files: &[&'a SourceFile]) -> BTreeMap<&'a str, BTreeSet<&'a str>> {
