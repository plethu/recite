...
fn authority_source_is_identity_even_when_overlay_bytes_match_saved_bytes() {
    let saved_request = BuildRequest::new_with_policy(
        ...
        recite_compiler::authoring::SnapshotGeneration::new(5),
    ...
    assert_ne!(
        recite_compiler::authoring::BuildRequestIdentity::from_request(&saved_request),
        recite_compiler::authoring::BuildRequestIdentity::from_request(&overlay_request)
    );

    struct SavedCheck {
        check: BuildCheck,
    }
    impl recite_compiler::authoring::BuildEngine for SavedCheck {
        fn check(
            &mut self,
            _: &BuildRequest,
    ...
    assert!(matches!(
        ...
        Some(recite_compiler::authoring::BuildResultFailure::Check(
            recite_compiler::authoring::BuildCheckError::RequestMismatch
        ))
    ));
    assert_eq!(check_publisher.commit_calls, 0);

    let fence = recite_compiler::authoring::BuildAuthorityFence::new(BuildAuthority::from_request(
        &saved_request,
    ));
    ...
    assert!(matches!(
        ...
        recite_compiler::authoring::PublishOutcome::Refused {
            reason: recite_compiler::authoring::PublishRefusal::StaleFingerprints
        }
    ));
