...
impl BuildEngine for ProjectBuildEngine {
    fn check(&mut self, _request: &BuildRequest, _control: &BuildControl) -> BuildCheck {
        BuildCheck::new(
            ...
            recite_compiler::authoring::FreshnessAssessment::not_assessed(
                self.request.fingerprints().clone(),
            ),
        )
    }
    ...
    fn build(
        ...
        for target in &self.targets {
            ...
            let report = match schema {
                Some(schema) => recite_compiler::compile::compile_inputs_with_schema(
                    inputs.clone(),
                    options,
                    schema,
                ),
                None => recite_compiler::compile::compile_inputs(inputs.clone(), options),
            }
