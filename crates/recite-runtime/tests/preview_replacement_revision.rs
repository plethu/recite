...
use recite_core::canonical_source_fingerprint;
...
fn same_id_revisions_clear_when_the_active_payload_returns() {
    ...
    let output = preview.assess_asset(&replacement);
    assert!(matches!(
        output.events(),
    ...
    let output = preview.assess_asset(&active);
...
fn same_id_candidate_revision_updates_and_round_trips() {
    ...
    preview.assess_asset(&replacement_b);
    let first = preview.state().restart_required().cloned().expect("first");
    preview.assess_asset(&replacement_c);
...
fn revision_asset(source_revision: &str, line: &str) -> recite_core::CompiledDialogue {
    ...
    asset.sources[0].fingerprint = canonical_source_fingerprint(source_revision);
    asset
}
