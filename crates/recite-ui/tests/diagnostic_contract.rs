...
#[test]
fn vscode_diagnostic_export_matches_executable_typed_contract() {
    let mut expected =
        String::from("# Generated by export-vscode-diagnostic-contract; do not edit.\n");
    for resource in recite_ui::all_resource_specs()
        .into_iter()
        .filter(|resource| {
            resource.id.as_str().starts_with("diagnostic-")
        ...
        })
    {
        if resource.arguments.is_empty() {
            expected.push_str(resource.id.as_str());
            expected.push('\n');
        } else {
    ...
    assert_eq!(
        expected,
        include_str!("../resources/vscode-diagnostic-contract.tsv"),
        "the checked-in VS Code diagnostic contract must come from UiContract::default"
    );
}
