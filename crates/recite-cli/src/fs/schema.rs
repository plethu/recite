...
pub(crate) fn load_schema_for_freshness(schema_path: &Path) -> Result<LoadedSchema, CliError> {
    let source = fs::read_to_string(schema_path).map_err(|source| CliError::Read {
        path: schema_path.to_owned(),
        source,
    })?;
    let report = load_schema_manifest_for_freshness_str(display_path(schema_path), &source);
    Ok(LoadedSchema {
        schema: report.schema,
        diagnostics: report.diagnostics,
    })
