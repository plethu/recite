...
# Closures, macros, and bare expression scopes are intentionally ambiguous;
...
#[allow(dead_code, reason = "macro definition is an opaque token boundary")]
macro_rules! generated { () => {} }
#[allow(dead_code, reason = "macro invocation is an opaque token boundary")]
generated!();
EOF
git -C "$test_root/repo" add .
git -C "$test_root/repo" commit -q -m ambiguous-scopes
ambiguous_sha="$(git -C "$test_root/repo" rev-parse HEAD)"
check_passes "$structured_sha" "$ambiguous_sha"
